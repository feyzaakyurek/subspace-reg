...
EXP_FOLDER=$DUMPED_PATH/"finetune_label_pull_pretrain_label_pull"
...
for FPULL in 0.02 0.03 0.0 0.04 0.1; do #5
(( cnt++ ))
if [[ $cnt -eq $SLURM_ARRAY_TASK_ID ]]; then
    BACKBONE_PATH="${DUMPED_PATH}/backbones/linear/label_pull/no_synonyms_label_pull_${PPULL}/resnet12_last.pth"
    EXP_NAME=lambda_${LMBD}_trloss_${TRLOSS}_fpull_${FPULL}_regularize_ppull${PPULL}
...
#                            --target_train_loss 1.1 > ppull_deneme.out
