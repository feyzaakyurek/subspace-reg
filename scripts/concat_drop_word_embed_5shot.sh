...
#SBATCH --job-name=query_size


DUMPED_PATH="/home/gridsan/eakyurek/gitother/rfs-incremental/dumped"
EXP_FOLDER=${DUMPED_PATH}/"finetune_label_attention_concat_drop_diagreg_query_size"
...
for TRLOSS in 0.55 0.85 1.0; do
for LR in 0.002 0.003; do
(( cnt++ ))
if [[ $cnt -eq $SLURM_ARRAY_TASK_ID ]]; then
    EXP_NAME=lambda_${LMBD}_trloss_${TRLOSS}_lr_${LR}
    BACKBONE_PATH=${DUMPED_PATH}"/backbones/c-x-concat/size_experiments/attention_concat_multipfc_${MULTIPFC}_diagreg_${DIAG_REG}_querysize_${QUERY_SIZE}/resnet12_miniImageNet_lr_0.05_decay_0.0005_trans_A_trial_pretrain_classifier_lang-linear_multip_${MULTIPFC}_8774292/resnet12_last.pth"
