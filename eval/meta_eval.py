...
#         # To save preds (temporary, comment out above later) #
        ...
        # IF HIERARCHICAL:
        if opt.hierarchical_eval:
            
            support_xs = support_xs.float().cuda()
            support_ys = support_ys_id.cuda()
            ...
            query_xs = query_xs.float().cuda()
            query_ys = query_ys_id.cuda()
            query_features = net(query_xs).view(query_xs.size(0), -1)
            
            # Retrive base query features. Support = query for base.
            base_support_xs, base_support_ys, *_ = drop_a_dim(base_batch)
            base_support_xs = base_support_xs.cuda()
            base_query_features = net(base_support_xs).detach().cpu().numpy()
            num_base = 64 if opt.dataset == "miniImageNet" else 200
            assert opt.dataset in ["miniImageNet", "tieredImageNet"]
            
            # Check support ys (is it 0, 1, ..)
            thres = np.max(support_features[:,:num_base])
            novel_is_base = (np.sum(query_features > thres, 1) >= 1) * 1.
            base_is_base = (np.sum(base_query_features > thres, 1) >= 1) * 1
            ...
            novel1 = np.mean(novel_preds == query_ys)
            base1 = np.mean(base_preds == base_query_ys)
            novel_hacc.append(novel1)
            base_hacc.append(base1)
            ...
            print('Hierarchical Evaluation \t'
                  ...
                  'Base@1 {:10.3f}\t'
                  'Avg@1 {:10.3f}\t'
                  'Running {:10.3f}\t'.format(idx,
                                            opt.neval_episodes,
                                            novel1, 
                                            base1,
                                            (novel1 + base1) / 2,
                                            (np.mean(novel_hacc) + np.mean(base_hacc))/2))
...
#     for idx, data in enumerate(meta_valloader):
    for idx in range(opt.neval_episodes):
        ...
        novel_embeds = dummy_classifier.embed.detach().cuda()
        novel_classifier_weights = novel_embeds  @ trns

        # Update the trained classifier of the network to accommodate for the new classes

        net.classifier = nn.Linear(640, len(vocab_all), bias=opt.lang_classifier_bias) # TODO!!
        net.classifier.weight = nn.Parameter(torch.cat([opt.orig_alpha * orig_classifier_weights, novel_classifier_weights], 0))
        # classifier.embed = nn.Parameter(torch.cat([embed, novel_embeds], 0),
        #                                 requires_grad=False) # TODO:CHECK DIM.
...
#         orig_transform_W = ckpt['model']['classifier.transform_W'] if opt.lmbd_reg_transform_w else None
        ...
        while train_loss > opt.target_train_loss or epoch < opt.novel_epochs + 1: #
            ...
            print('Base\tAcc@1 {:10.3f}\t'.format(acc_base_))
            print('Avg\tAcc@1 {:10.3f}\t'.format(avg_score))
            epoch += 1
